...
#define USER_AVATAR @"account_avatar_onlinecustomer"
#define SERVER_AVATAR @"account_avatar_onlineservice"
#define SnowFundPreix @"来自苹果手机: "
...
//    [EMCDDeviceManager sharedInstance].delegate = self;
    ...
    [[EaseBaseMessageCell appearance] setSendBubbleBackgroundImage:[[UIImage imageNamed:@"account_onlineservice_dialog_orange"] stretchableImageWithLeftCapWidth:5 topCapHeight:35]];
    [[EaseBaseMessageCell appearance] setRecvBubbleBackgroundImage:[[UIImage imageNamed:@"account_onlineservice_dialog_white"] stretchableImageWithLeftCapWidth:35 topCapHeight:35]];
    ...
    [self tableViewDidTriggerHeaderRefresh];
      ...
      // custom
    NSString *userInfo = [[NSUserDefaults standardUserDefaults] stringForKey: @"UserInfoMessage"];
    NSUInteger count = self.navigationController.viewControllers.count;
    if (count > 1) {
      NSString *from = self.navigationController.viewControllers[count - 2].description;
      userInfo = [userInfo stringByAppendingString: from];
    }
    NSString *sys = [NSString stringWithFormat: @"sys: %@, app: %@\n",
                     [UIDevice currentDevice].systemVersion,
                     [NSBundle mainBundle].infoDictionary[@"CFBundleShortVersionString"]];
    self.chatId = [NSString stringWithFormat: @"%@ %@ %@", SnowFundPreix, sys, userInfo];
    [self sendTextMessage: self.chatId];
}
...
- (NSArray *)formatMessages:(NSArray *)messages
{
    ...
    for (EMMessage *message in messages) {
        ...
        if (_dataSource && [_dataSource respondsToSelector:@selector(messageViewController:modelForMessage:)]) {
          ...
          NSString *avatarName = message.direction == EMMessageDirectionSend ? USER_AVATAR : SERVER_AVATAR;
          model.avatarImage = [UIImage imageNamed: avatarName];
            model.failImageName = @"imageDownloadFail";
        }

      if (model.bodyType == EMMessageBodyTypeText && [model.text hasPrefix: SnowFundPreix]) {
        continue;
      }
...
- (void)_updateMessageStatus:(EMMessage *)aMessage
{
    ...
    if (aMessage && isChatting) {
        ...
        if (_dataSource && [_dataSource respondsToSelector:@selector(messageViewController:modelForMessage:)]) {
            ...
            NSString *avatarName = aMessage.direction == EMMessageDirectionSend ? USER_AVATAR : SERVER_AVATAR;
            model.avatarImage = [UIImage imageNamed: avatarName];
            model.failImageName = @"imageDownloadFail";
        }
        if (model) {
